...
namespace ParserCore
{
    ...
    class EnumerableTypeParser : CsvParser
    {
        EnumerableType GetEnumableType(Type type)
        {
            ...
            else if (type.IsGenericType) return EnumerableType.KeyValuePair;
        ...
        public void SetValue(object obj, FieldInfo info, string[] values)
        {
            ...
            {
                case EnumerableType.Array: new CsvArrayParser().SetValue(obj, info, GetValues(values)); break;
                case EnumerableType.List: new CsvListParser().SetValue(obj, info, GetValues(values)); break;
                case EnumerableType.Dictionary: new CsvDictionaryParser().SetValue(obj, info, GetValues(values)); break;
                case EnumerableType.KeyValuePair: new CsvPairParser(info.FieldType).SetValue(obj, info, GetValues(values)); break;
            }
        }
    ...
    public class CsvPairParser : ICsvIEnumeralbeParser
    {
        Type _type;
        ...
        public void SetValue(object obj, FieldInfo info, string[] values)
        {
            ...
            object value = constructor.Invoke(new object[] { PrimitiveTypeParser.GetPrimitiveParser(elementTypes[0]).GetParserValue(values[0]),
    ...
    public static class TypeIdentifier
    {
        ...
        public static bool IsIEnumerable(Type type) 
            => type.IsArray || IsList(type) || IsDictionary(type) || (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(KeyValuePair<,>));
        public static bool IsCustom(Type type)
        {
            if (type.IsEnum || type == typeof(UnitFlags) || type == typeof(List<UnitFlags>) || type == typeof(UnitFlags[])) return false;
