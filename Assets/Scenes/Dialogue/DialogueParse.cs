...
public struct DEBUGTEXT
{
    ...
    public TalkData[] tds;
    public DEBUGTEXT(string name, TalkData[] td)
    {
        eventName = name;
        tds = td;
    }
...
public class DialogueParse : MonoBehaviour
{
    ...
    private static Dictionary<string, TalkData[]> TalkDictionary = new Dictionary<string, TalkData[]>();
    public static TalkData[] GetDialogue(string eventName)
    {
        if(TalkDictionary.ContainsKey(eventName)) return TalkDictionary[eventName];
        else
        {
    ...
    [ContextMenu("디버그 대사 세팅")]
    void SetDEBUGTEXT()
    {
        List<string> eventNames = new List<string>(TalkDictionary.Keys);
        List<TalkData[]> talkDatasList = new List<TalkData[]>(TalkDictionary.Values);

        for(int i = 0; i < eventNames.Count; i++)
        {
            DEBUGTEXT debugTalk = new DEBUGTEXT (eventNames[i], talkDatasList[i] );
            DEBUGTEXTS.Add(debugTalk);
        }
    }
    ...
    public void SetTalkDictionary()
    {
        ...
        for (int i = 1; i < datas.Length; i++) // 엑셀 파일 1번째 줄은 편의를 위한 분류이므로 i = 1부터 시작
        {
            ...
            string eventName = row[0];
            TalkData[] talkDatas = GetTalkDatas(datas, ref i);
            TalkDictionary.Add(eventName.Trim(), talkDatas);
        }
    }

    TalkData[] GetTalkDatas(string[] datas, ref int index)
    {
        string[] row = datas[index].Split(new char[] { ',' });
        List<TalkData> talkDataList = new List<TalkData>();
        ...
        while (row[0].Trim() != "end") // talkDataList 하나를 만드는 반복문
        {
            ...
            {
                contextList.Add(row[2].ToString());
                if (++index < datas.Length) row = datas[index].Split(new char[] { ',' });
                else break;
            } while (row[1] == "" && row[0] != "end");
